...
//Function that will apply past,present and future classes to each time block   
  function checkTime() {
    var getTime = dayjs(). hour(); //use dayjs to get current time
    ...
    var timeBlockHours = parseInt($(this).attr("id").split("-")[1]); // extracts the hour by splitting the the ID string and prasing it as an integer 
    var descriptionId = localStorage.getItem(timeBlock);

    if (descriptionId) {
      $(this).find(".description").val(descriptionId);
    }
    if (timeBlockHours < getTime) {
      $(this).addClass("past");
    } else if (timeBlockHours == getTime) {
